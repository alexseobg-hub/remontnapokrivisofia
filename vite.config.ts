...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      /*
       * Картите на кода се качват заедно с него. Сгъстеният файл е нечетим,
       * а с тях всяка грешка сочи към истинския ред.
       *
       * Не тежат на посетителя: браузърът ги дърпа само когато някой отвори
       * инструментите за разработка. Изходният код така си стои открит, но
       * тук няма какво да се крие — сайтът е текст, стилове и една форма.
       */
      sourcemap: true,
      rollupOptions: {
        output: {
